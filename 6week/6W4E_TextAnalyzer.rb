...
class Analyze < Display
  ...
  def character_ct_no_spaces 
    ...
    display_result(@character_ct_no_spaces) {"characters, without spaces,"}
  end

  def word_ct
    @word_ct = @file_contents.join.delete("^a-zA-Z|\s").split(" ").length
    display_result(@word_ct) {"words"}
  end

  def paragraph_ct
    @paragraph_ct = @file_contents.join.split(/\n\n/).length
    display_result(@paragraph_ct) {"paragraphs"}
  end

  def sentences_ct
    @sentences_ct = @file_contents.join.split(/\.|!|\?/).length
    display_result(@sentences_ct) {"sentences"}
  end

  def avg_words_per_sent
    @avg_words_per_sent = @word_ct / @sentences_ct
    display_result(@avg_words_per_sent) {"words per sentence, on average,"}
  end

  def avg_sent_per_paragraph 
    @avg_sent_per_paragraph = @sentences_ct / @paragraph_ct
    display_result(@avg_sent_per_paragraph) {"sentences per paragraph, on average,"}
  end
end
